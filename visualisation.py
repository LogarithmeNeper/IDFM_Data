...
def draw_map(data: pd.DataFrame, folder: str, name: str) -> None:
    ...
    data = data[['route_id', 'route_long_name', 'stop_id', 'stop_name', 'stop_lon', 'stop_lat']]
    for row in data.iterrows():
        row_values = row[1]
        #folium.Marker([row_values['stop_lat'], row_values['stop_lon']], popup=row_values['stop_name']).add_to(map)
        folium.Marker([row_values['stop_lat'], row_values['stop_lon']], popup=row_values['stop_name'], icon=folium.Icon(color=global_variables.COLOURS_METRO_LINES[row_values['route_id']])).add_to(map)
...
filtered_data = explorer.filter_lines_by_id(data, global_variables.METRO_LINES)
